...
static const CLIENT::ACTION SBA_SUBSCRIBEX
(
   SBA_SUBSCRIBE,
   "{"
      "\"wCount\"                : \"WORD\","
      ...
      "["
         "{"
            ...
            "\"wClass\"                :  \"WORD\""
         "},"
         "256," // need a #define for max number of subscriptions
         "\"wCount\""
      "]"
   "}",
   "{"
...
public:
   ...
   void Unlock ()
   {
      ...
      if (bLockCount == 0 && bDirty != 0)
      {
         for (bSubscription = 0; bSubscription < aSubscription.size (); bSubscription++)
         {
            Subscription = aSubscription[bSubscription];

            if (Subscription.nCount == 0 && (Subscription.bFlag & (SUBSCRIPTION::eFLAG::SUBSCRIBING | SUBSCRIPTION::eFLAG::SUBSCRIBED | SUBSCRIPTION::eFLAG::UNSUBSCRIBING)) == 0)
            {
               Delete (&Subscription);
   ...
   void DeleteEx (SUBSCRIPTIONDATA* Subscription)
   {
      if (bLockCount == 0)
      {
         Delete (Subscription);
...
bool SUBSCRIPTION::Add (uint16_t wClass, uint64_t twObjectIx)
{
   bool bResult = false;
   SUBSCRIPTIONDATA *pSubscription;
...
bool SUBSCRIPTION::Remove (uint16_t wClass, uint64_t twObjectIx)
{
   bool bResult = false;
   SUBSCRIPTIONDATA* pSubscription;
...
bool SUBSCRIPTION::Reset (uint16_t wClass, uint64_t twObjectIx)
{
   bool bResult = false;
   SUBSCRIPTIONDATA* pSubscription;
...
void SUBSCRIPTION::Subscribe_Aux ()
{
   int bSubscription;
   SUBSCRIPTIONDATA* pSubscription;
   std::vector<SBA_SUBSCRIBE_EX_IN> aSBA_Subscribe_Ex_In;
   SBA_SUBSCRIBE_EX_IN SBA_Subscribe_Ex_In;
   int wState;

   {
      std::lock_guard<std::recursive_mutex> guard (m_pImpl->m_CS);
      {
         for (bSubscription = 0; bSubscription < m_pImpl->aSubscription.size (); bSubscription++)
         {
            ...
            wState = SBA_SUBSCRIBE_STATE_COMPLETE;

            if (pSubscription->nCount > 0)
            {
               if ((pSubscription->bFlag & (eFLAG::SUBSCRIBED | eFLAG::SUBSCRIBING)) == 0
                  || (pSubscription->bFlag & eFLAG::UNSUBSCRIBING) != 0)
               {
                  pSubscription->bFlag &= ~eFLAG::UNSUBSCRIBING;
                  pSubscription->bFlag |= eFLAG::SUBSCRIBING;

                  wState = SBA_SUBSCRIBE_STATE_RECOVER_SELF;
               }
               else if ((pSubscription->bFlag & eFLAG::RESET) != 0)
                  wState = SBA_SUBSCRIBE_STATE_RESET;
            }
            else
            {
               if ((pSubscription->bFlag & (eFLAG::SUBSCRIBED | eFLAG::UNSUBSCRIBING)) == eFLAG::SUBSCRIBED
                  || (pSubscription->bFlag & eFLAG::SUBSCRIBING) != 0)
               {
                  pSubscription->bFlag &= ~eFLAG::SUBSCRIBING;
                  pSubscription->bFlag |= eFLAG::UNSUBSCRIBING;

                  wState = SBA_SUBSCRIBE_STATE_REMOVE;
               }
            }

            if (wState != SBA_SUBSCRIBE_STATE_COMPLETE)
            {
               SBA_Subscribe_Ex_In.wState     = wState;
               SBA_Subscribe_Ex_In.wClass     = pSubscription->wClass;
               SBA_Subscribe_Ex_In.twObjectIx = pSubscription->twObjectIx;

               aSBA_Subscribe_Ex_In.push_back (SBA_Subscribe_Ex_In);
            }
         }
      }
   }

   if (aSBA_Subscribe_Ex_In.size () > 0)
   {
      RMAP::CORE::CLIENT::IACTION* pIAction = m_pImpl->Request ("SUBSCRIBE");
      CLIENT::IACTION* pIActionSB = dynamic_cast<CLIENT::IACTION*> (pIAction);

      ordered_json& pRequest = pIActionSB->GetRequest ();

      pRequest["wCount"] = aSBA_Subscribe_Ex_In.size ();

      for (size_t i = 0; i < aSBA_Subscribe_Ex_In.size (); i++)
      {
      ...
      if (pIActionSB->Send (this->m_pImpl, 0, NULL) == false)
      {
         pIActionSB->SetResult (SBA_RESULT_TRANSMITFAILURE);
      }
   }
}
